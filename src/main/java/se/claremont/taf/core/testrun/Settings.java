...
public class Settings extends HashMap<String, String> {
    ...
    private void readFromFileIfItExistElseTryToCreateFile(String settingsFilePath) {
        ...
        if (settingFile.exists()) {
            ...
            System.out.println("Could not read TAF settings from file '" + settingsFilePath + "'. Don't worry. Continuing with default values, and attempting saving of the settings to file '" + settingsFilePath + "' for next time.");
            try {
                writeSettingsParametersToFile(settingsFilePath);
                setValue(SettingParameters.RUN_SETTINGS_FILE, settingsFilePath);
            } catch (Exception ex) {
                System.out.println("Could not save Settings to new settings file '" + settingsFilePath + "'. Error message: " + ex.getMessage());
            }
        }
    }
